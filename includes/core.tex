...
Použitý zápis:
\begin{lstlisting}[style=code-listing, caption={LaTeX zápis matematického výrazu}, label={fig:admin_edit}]
...
Knižnica MathQuill je závislá na knižnici \textbf{jQuery}, ktorá zabezpečuje asynchrónnu manipuláciu s HTML a CSS prvkami stránky, ako aj jednoduchú prácu s animáciami a DOM elementmi.
 Spojením týchto technológií sme vytvorili plynulé a responzívne prostredie pri práci s matematickými výrazmi.
 ...
 Dynamicky generované úlohy sú následne vložené do tabuľky, kde každú riadok reprezentuje jednu úlohu.
...
Po odoslaní požiadavky na vytvorenie novej úlohy smerom k serveru sa táto spracováva v definovanej API route na strane backendu (v súbore \textit{server.js}). 
...
V prípade úspechu sa späť do klienta odosiela JSON odpoveď s detailmi vytvorenej úlohy.
